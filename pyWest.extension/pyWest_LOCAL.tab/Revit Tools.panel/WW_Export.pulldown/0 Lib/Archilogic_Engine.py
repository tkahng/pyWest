...
class ExportRevit:
    def __init__(self, doc, app=None, filePath=None, fileName=None, ifcOptions=None, checkOutput=False):
        ...
        if not self.fileName:
            self.fileName = self.doc.Title
        self.ifcOptions = ifcOptions
        if not self.ifcOptions:
            self.ifcOptions = self.ArchilogicOptions() # WW default export options      
    ...
    def ExportRVT(self):
        ...
        self.doc.Export(self.filePath, self.fileName, self.ifcOptions) # Autodesk.Revit.DB.Document.Export("folder","name",options)

        t.Commit() # transaction must be committed before proceeding to linked files    
