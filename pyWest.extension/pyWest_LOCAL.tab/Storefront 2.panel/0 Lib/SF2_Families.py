    ...
    def CheckFamilyDict(self, checkDict, quickLoad=False):
        ...
        try:
            for famName, symbolAndTypes in checkDict.items():
                famFound = False
                for familyId in SF2_Utility.GetAllElements(self.doc, None, Autodesk.Revit.DB.Family):
...
######################################
## READ/WRITE FAMILY CONFIGURATIONS ##
######################################
...
class LoadFamilies:
    ...
    def LoadRegularFamilies(self):
        #Load the necessary families - CLEAN THIS UP
        try:
            try:
                # google drive stream
            ...
            # last resort look for familes locally - will be deprecated
        ...
        self.profileDict = SF2_Utility.GetProfileDict("I-Profile-Storefront-Mullion")
        self.wallTypeDict = SF2_Utility.GetWallTypeDict()
        self.mullionDict = SF2_Utility.GetMullionTypeDict(self.doc)
        self.panelTypeDict = SF2_Utility.GetWindowTypeDict()
        self.quadMullionDict = SF2_Utility.GetQuadMullionTypeDict()
        createMullions = {}
    def LoadMullionFamilies(self):
        ...
        if self.quadMullionDict.keys() and self.mullionDict.keys():
            ...
            with rpw.db.Transaction("Create Curtain Wall") as tx:
                SF2_Utility.SupressErrorsAndWarnings(tx) # i added this and the "as tx" above
...
#######################################
## PARENT CLASS, MOST FAMILY         ##
## CLASSES INSTANTIATED THROUGH HERE ##
#######################################
